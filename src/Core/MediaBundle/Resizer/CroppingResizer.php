...
namespace Core\MediaBundle\Resizer;
...
class CroppingResizer implements ResizerInterface
{
    protected $adapter;
    protected $mode;
    protected $metadata;
    ...
    public function resize(MediaInterface $media, File $in, File $out, $format, array $settings)
    {
        ...
        $format = 'jpeg'; //force JPEG format, even if the original extension is PNG (will still save .png file, which can cause bugs)
        $content = $image
            ->thumbnail($this->getBox($media, $settings), $this->mode)
            ->get(
                $format,
                $format === 'png'
                    ? array('png_compression_level' => (integer)round($settings['quality']/10))
                    : array('quality' => $settings['quality'])
            );
